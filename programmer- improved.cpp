...
void wait()
{
	...
	c = getch();//程序自动读取一个字符，无需按回车也不显示在屏幕上
}
...
void SearchWord(int num, struct word* p)
{
	...
	int flag;
	char str[20];
	int order[26] = { 0 };//首字母所在位置
	int order1[26] = { 0 };
	int place, letter;
	for (i = 0; i < num; i++)
	{
		letter = (p + i)->english[0] - 97;
		if (letter < 0)
		{
			letter = (p + i)->english[0] - 65;
		}
		order[letter]++;
	}
	order1[0] = 0;
	...
	while (1)
	{
		...
		else if (x == 1)
		{
			...
			place = order1[letter];
			for (i = place; i < order[letter] + place; i++)
			{
